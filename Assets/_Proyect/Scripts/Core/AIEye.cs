...
public class AIEye : MonoBehaviour
{
    ...
    // HealthBase del jugador detectado; solo esta clase puede asignarlo (setter privado)
    ...
    void Scan()
    {
        ...
        for (int i = 0; i < _count; i++)
        {
            ...
            {
                float dist = Vector3.Distance(transform.position, _colliders[i].transform.position);
                if (dist < minDistance)
                {
                    minDistance = dist;
                    // Almacena el transform del jugador detectado
                    ViewEnemy = _colliders[i].transform.GetComponent<HealthBase>();
                }
            }
        }
    ...
    void OnDrawGizmos()
    {
        ...
        if (ViewEnemy != null && positionEye != null)
        {

            if (!Physics.Linecast(positionEye.position, ViewEnemy.transform.position, occlusionLayers))
            {
                Gizmos.color = Color.blue;
                Gizmos.DrawLine(positionEye.position, ViewEnemy.transform.position);
